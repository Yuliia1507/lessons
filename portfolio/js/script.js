...
swiper.el.addEventListener('touchend', function (e) {
	if (swiper.isEnd) {
		// Scroll down when at the last slide
		window.scrollBy(0, 10);
	} else if (swiper.isBeginning) {
		// Scroll up when at the first slide
		window.scrollBy(0, -10);
	}
});
...
swiper.el.addEventListener('touchend', function (e) {
	if (swiper.isEnd) {
		window.scrollBy(0, 10); // Scroll down when at the last slide
	} else if (swiper.isBeginning) {
		window.scrollBy(0, -10); // Scroll up when at the first slide
	}
});
