	...
	elementsToMove.forEach(function (element) {
		...
		if (data.length === 3) {
			...
			if (screenWidth <= requiredScreenWidth && !element.classList.contains("moved")) {
				...
				if (initialParent) {
					let initialParentElement = document.querySelector(".header__link"); // Отримати початкового батька напряму
...
document.querySelectorAll('.color-palette__btn').forEach(option => {
...
document.documentElement.style.setProperty('--accent-color', '#47bbfc');
//==================================
...
window.onload = function () {
	const elements = document.getElementsByClassName('hero__typewriter', 'skills__title');
...
const swiper2 = new Swiper('.testimonials__swiper', {
	...
	autoplay: {
		delay: 5000, // Затримка перед переключенням слайда (в мілісекундах)
		disableOnInteraction: false, // Автопрокрутка не зупиняється після взаємодії користувача
	},
});
