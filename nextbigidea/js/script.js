...
if (popupCloseIcon.length > 0) {
	for (let index = 0; index < popupCloseIcon.length; index++) {
		...
		el.addEventListener('click', function (e) {
			const popup = el.closest('.popup'); // Знаходимо найближчий батьківський елемент з класом 'popup'
			popupClose(popup); // Викликаємо функцію для закриття попапа
			e.preventDefault();
		});
	}
}
...
function getScrollbarWidth() {
	// Створюємо елемент для визначення ширини скролбару
...
function bodyLock() {
	...
	const lockPaddingValue = scrollbarWidth + 'px';  // додаємо ширину скролбару
	...
	// Перевірка наявності прокручування
...
function bodyUnLock() {
	setTimeout(function () {
		...
		body.style.overflow = ''; // Відновлюємо стандартний стан прокручування
	}, timeout);
}
